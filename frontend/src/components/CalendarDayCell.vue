    ...
    <div class="day-events">
      ...
      >
        ...
        <span class="event-copy">
          ...
          <small><span aria-hidden="true">✓</span> Completed · {{ metric(activity) }}</small>
        </span>
      </router-link>
...
const showPlan = computed(() => props.plan && !['linked', 'matched'].includes(props.plan.comparison?.status))
...
const status = computed(() => props.plan?.comparison?.status)
...
const metric = (item) => item.duration_min ? `${Math.round(item.duration_min)} min` : item.distance_km ? `${item.distance_km} km` : 'Details'
...
const planStatusTone = (item) => {
  ...
  if (['replaced', 'moved', 'partially_matched', 'rest_day_changed'].includes(value)) return 'changed'
