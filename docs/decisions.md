...
- Implemented the Plan visual redesign as one horizontal, sport-colored week board with a single date/focus heading and compact workload strip. Removed the duplicate week navigator, featured-session overview, and agenda heading. Session details open on demand; weekly coaching notes are collapsible and recent execution context sits below the board. Weather, activity linking, protected-day editing, and coaching approval controls remain available. The board scrolls horizontally on smaller screens.
- Plan uses a restrained type scale and inline weekly totals. Each day exposes an explicit text-and-symbol execution badge (linked, automatically matched, partial, different, unmatched, or upcoming) independent of its sport-colored border. Editing controls live under Manage week; notes, goals, and collapsed revision history follow the session board. Revision history expands into compact rows rather than a prominent timeline.
- Implemented the athlete-focused Trends redesign: the overview leads with an interactive 4/8/12-week volume timeline calculated from available current-year workout-calendar data, with hours/sessions selection and weekly inspection. The current week is explicitly partial; rest days are not penalized.
- Recovery now includes a selectable-night sleep-composition ring and Deep, REM, Core, unspecified sleep, and separate awake durations. These are imported stage totals, not a fabricated overnight timeline; missing stage detail is shown explicitly.
- Recovery’s sleep-duration trend now renders imported Deep, REM, Core, and unspecified sleep as stacked nightly bars, with stage colors and stage-aware inspection text. Nights without stage data fall back to a single total bar.
- Daily activity now highlights the selected metric with its latest value and seven recorded days of movement. Full training-load analysis emphasizes the adaptation chart and large fitness/fatigue/form values, retaining existing load calculations and supporting coverage, ratio, and focus information.
...
- Planned-workout details use a compact sport-accented brief with one duration/distance/effort/zone row, readable instructions, visible adaptation guidance, and collapsed goal/plan context. Intensity extraction preserves hyphenated and typographic-dash ranges; goal support text is deduplicated. Keyboard focus enters the modal, stays inside while open, and returns to its trigger on close.

- The horizontal Plan board shares natural header, planned-session, and completed-activity row heights across all days using CSS subgrid. Planned and completed sections align across columns; additional activities extend downward without shifting the section start. No fixed content heights or clipping are used for alignment.

- Plan forecasts sit compactly beside the day/date instead of occupying a separate header row. Past days therefore need no weather spacer; shared planned/completed alignment is preserved. Full forecast wording remains available through the weather indicator’s accessible label and tooltip.

- Dashboard visual redesign: today’s training decision is the primary sport-accented surface, with measured load in a narrower, unboxed sidebar. The workout retains its sport icon, strongest color, and primary action. Coaching interpretation occupies its own row beneath both panels, with assessment and next step side by side. Session instructions are visible beside the workout targets and action when the card has sufficient width, and stack below on smaller cards; completed activities remain accessible. The week uses a compact horizontal strip and inline actual totals; yearly charts and exploration links use lighter surfaces with a consistent type scale. Existing daily-state, coaching refresh, and explicit plan-adaptation behavior is preserved.

- Dashboard distinguishes the saved workout from the coach’s assessment: the neutral workout badge says Saved plan rather than Stay on plan, and an assessment link leads to the coaching row when available. Recovery and caution states remain explicit.

- Strength now defaults to a concise hybrid-athlete overview: resume an active workout or review the next saved strength day, then inspect session frequency, the most recent linked workout, and one recurring lift. Workout context loads independently of analysis filters and falls back to the library when unavailable. Lift progression, session history, and training-volume/body-part analysis have separate tabs. Best recorded loads are labeled as window-specific rather than all-time PRs; volume is not treated as a readiness score. Existing set history and studio workflows remain available.

- Strength detail tabs now have distinct layouts: searchable lift selection beside a top-load/volume chart and session comparison table; a searchable dated session log with expandable sets; and weekly session/set/volume analysis followed by movement distribution and window-specific best loads. Next-workout content stays on Overview. Missing top loads are omitted from the load chart rather than replaced with volume values, keeping units consistent.

- Strength ranges include 26 weeks (6 months) and 52 weeks (Past year), alongside 4/8/12 weeks. These are rolling weekly windows, not calendar-year filters. Weekly charts scroll horizontally for extended ranges, and session history includes all linked sessions in the selected window rather than only the latest eight.
- Strength activity detail now leads with the recorded work: an exercise roster sits beside the selected lift’s warm-up and working-set log, with muscle coverage and heart-rate effort kept visible as compact supporting panels. This keeps set-level strength evidence visible without changing detail layouts for other sports.

- Completed dashboard days show actual duration, distance, and session count beside a bounded, scrollable activity list; planned targets and pre-session context are omitted in this state. Virtual rides use the ride icon.
- The dashboard Codex helper explicitly selects gpt-5.6-luna for coaching, activity analysis, and planning, with gpt-5.6-terra as its default capacity fallback, rather than inheriting the user’s global CLI model. The health endpoint reports this primary model.

- Data & Sync uses a persistent four-source selector with readiness, pending counts, and distinct source accents above one focused import workspace. Optional date ranges, file lists, import history, and exercise previews expand on demand. Fitbod review remains actionable by default. Scan refreshes preserve completed import feedback, and independent source-loading failures do not block the rest of the page.

- Coach Notes is a read-only coaching journal with category filters, search across loaded notes, an expandable archive, compact previews, and a dedicated full-note reader. Mobile previews scroll horizontally above the reader. Notes retain their saved content and dates; loading, errors, empty categories, and no search results have distinct states. Category requests discard stale responses.
